    ...
    var body: some View {
        Button(action: onToggle) {
            HStack(spacing: 12) {
                ...
                .onAppear {
                    print("DEBUG: ExerciseRow - Exercise: \(exercise.name ?? "unnamed"), GIF URL: \(exercise.fullGifUrl ?? "nil")")
                }
    ...
    var body: some View {
        Button(action: onToggle) {
            HStack(spacing: 12) {
                ...
                // Selection indicator (inside HStack)
                if workoutBuilder.containsExercise(id: exercise.id) {
                    HStack {
                        ZStack {
                            Circle()
        ...
        .buttonStyle(PlainButtonStyle()) // ✅ No more error
        .onAppear {
